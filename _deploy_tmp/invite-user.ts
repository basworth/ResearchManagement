...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Named for a reason: `caller` is the getUser() envelope, not the user, and reaching for a
    // bare `callerId` (which is what the sibling admin-requests function calls it) threw
    // "callerId is not defined" at runtime in the cancel branch. One name, defined once, here.
    ...
    const { data: adminRow } = await admin.from("app_admins").select("user_id").eq("user_id", callerId).maybeSingle();
